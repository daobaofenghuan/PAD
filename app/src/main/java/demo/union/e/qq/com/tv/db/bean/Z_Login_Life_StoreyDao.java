package demo.union.e.qq.com.tv.db.bean;

import android.content.Context;
import android.util.Log;

import com.j256.ormlite.dao.Dao;


import java.util.List;
import java.util.UUID;

import demo.union.e.qq.com.tv.base.Z_DataBaseHelper;


/**
 * Created by luoliwen on 16/4/28.
 * 用户注册信息
 */
public class Z_Login_Life_StoreyDao {
    private Context context;
    private Dao<Life_Storey, Integer> userDao;
    private Z_DataBaseHelper helper;

    public Z_Login_Life_StoreyDao(Context context) {
        this.context = context;
        helper = Z_DataBaseHelper.getInstance(context);
        try {
            userDao = helper.getDao(Life_Storey.class);
        } catch (Exception e) {
            e.printStackTrace();
        }

    }

    public void addUser(String account
    ) {
        try {

            Life_Storey user = new Life_Storey();
            user.setId(UUID.randomUUID().toString());
            user.setAccount(account);
            userDao.create(user);
            Log.e("0388", "添加成功");

        } catch (Exception e) {
            e.printStackTrace();
            Log.e("0388", "添加失敗" + e.toString());
        }
    }


    public boolean shifoucunzai(String account) {
        try {
            List<Life_Storey> account1 = userDao.queryBuilder().where().eq("account", account).query();
            if (account1.size() > 0) {
                return true;
            } else {
                return false;
            }

        } catch (Exception e) {
            e.printStackTrace();
        }
        return false;
    }

    public void delete(String account) {
        try {
            Life_Storey account1 = userDao.queryBuilder().where().eq("account", account).query().get(0);
            userDao.delete(account1);

        } catch (Exception e) {
            e.printStackTrace();
        }

    }

    public String getID(String account) {
        try {
            Life_Storey account1 = userDao.queryBuilder().where().eq("account", account).query().get(0);
            return account1.getId();

        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    }

    List<Life_Storey> account1;

    public List<Life_Storey> quell() {
        try {
            account1 = userDao.queryForAll();
            return account1;

        } catch (Exception e) {
            e.printStackTrace();
            Log.e("1111", "xx2222" + e.toString());
        }
        return null;
    }
}
